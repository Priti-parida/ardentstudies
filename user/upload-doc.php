...
$pgra=md5($pgmarksheet).$extensionpgra;
    ...
    $query=mysqli_query($connn,"insert into tbldocument(UserID,TransferCertificate,TenMarksheeet,TwelveMarksheet,GraduationCertificate,PostgraduationCertificate) value('$uid','$tc','$tm','$twm','$gra','$pgra')");
    if ($query) {
...
<?php 
$stuid=$_SESSION['uid'];
$ret=mysqli_query($conn,"select AdminStatus from  tbladmapplications join tbluser on tbluser.ID=tbladmapplications.UserID where tbluser.ID='$stuid' ");
$num=mysqli_fetch_array($ret);
$adstatus=$num['AdminStatus'];
if($num>0 && $adstatus=='1' )
{

$query=mysqli_query($conn,"select * from tbldocument where  UserID=$stuid");
